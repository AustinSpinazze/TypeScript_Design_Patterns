...
str2 = 'ABC';
bool1 = str2 === 'ABC';
let bool2: boolean = 'abc'.endsWith('b');
...
let dictionary1: { [key: number]: string };
dictionary1 = { 123: 'abc', 456: 'def' };
let dictionary2: { [key: string]: boolean };
...
let dictionary3: { [key: string]: string };
let dictionary4: { [id: number]: string };
...
let tuple1: [number, string]; // Only two items, number and string
tuple1 = [1, 'abc'];
let tuple2: [number, string, boolean]; // Only three items, number, string and boolean
tuple2 = [1, 'abc', true];
...
let set1: Set<number>;
set1 = new Set<number>([1, 2, 3, 4, 5]);
let set2: Set<string>;
set2 = new Set<string>(['a', 'b', 'c', 'd', 'e']);
let set3: Set<unknown>;
set3 = new Set<unknown>([
	1,
	'a',
	true,
